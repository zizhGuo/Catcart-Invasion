...
public class MultiShooter : MonoBehaviour
{
    ...
    public static bool isPlayerShoot = false;
    ...
    public void shoot()
    {
        ...
        //if (GameManager.sPistolBulletIsLaser)
        {
            ...
            // If this shooter is from a taser shooter enemy
            if (GetComponentInParent<newNASTaserShooterEnemyBehavior>())
            {
                s1.GetComponent<BeamCollision>().taserBeamOwner = GetComponentInParent<newNASTaserShooterEnemyBehavior>();
                s1.GetComponent<BeamCollision>().taserBeamOwner.currentShot = s1;
            }
            else isPlayerShoot = true;
