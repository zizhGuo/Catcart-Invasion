...
public class NewBeamCollision : MonoBehaviour {

    [SerializeField] public static bool isShot = false;
    ...
    void Update()
    {
            ...
            Physics.Raycast(transform.position, transform.forward, out hit, Mathf.Infinity, shootLayer))
        {
            ...
            if (Mathf.Abs(hit.distance - BL.GetNowLength()) < 0.5f)
            {
                ...
                if (!hitobj.GetComponent<PlayerInfo>()) return;
                if (!effective) return;
