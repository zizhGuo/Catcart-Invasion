...
public class DroidSpawner : MonoBehaviour
{
    ...
    public int spawnRadiusMax;                  //The maximum distance away from the player where the drone should be spawned
    public int spawnRadiusMin;                  //The minimum distance away from the player where the drone should be spawned
    ...
    void Update()
    {
        if (Time.time - lastSpawnTime + ((betterRandom(0, Mathf.RoundToInt(spawnCD / 4000f * 3000f))) / 1000f) >= spawnCD)
        {
    ...
    public IEnumerator spawnEnemy()
    {
        ...
        Vector3 newLocation = new Vector3(betterRandom(-spawnRadiusMax, spawnRadiusMax), 0, betterRandom(-spawnRadiusMax, spawnRadiusMax));

        while (newLocation.magnitude < spawnRadiusMin || newLocation.magnitude > spawnRadiusMax || Vector3.Angle(transform.forward, newLocation) >= 30)
        {
            newLocation.x = betterRandom(-spawnRadiusMax, spawnRadiusMax);
            newLocation.z = betterRandom(-spawnRadiusMax, spawnRadiusMax);
        }
        ...
        Instantiate(enemyPrefab, new Vector3(newLocation.x, 0, newLocation.z), Quaternion.identity);
